...
function ColHeading({ college, children }) {
  return (
    <div className="mb-6">
      ...
      <div className="w-8 h-[2px] rounded-full" style={{ backgroundColor: college.greenAccent }} />
    </div>
  );
}
...
function FooterLink({ href, isExternal, college, children }) {
  ...
  const inner = (
    <>
      <span className="w-1 h-1 rounded-full flex-shrink-0 mt-[8px]" style={{ backgroundColor: college.greenAccent }} />
      <span>{children}</span>
    </>
  );
  if (isExternal) {
