// administrationData.js — content for all Administration section pages

// ── Profile pages ─────────────────────────────────────────────────────────────

export const chairmanData = {
  name: 'Dr. G. Ganga Raju',
  title: 'Chairman',
  institution: 'Gokaraju Rangaraju College of Pharmacy',
  img: 'https://grcp.ac.in/images/Gokaraju_Ganga_Raju.png',
  qualifications: [],
  bio: [
    'Inspired by a deep desire to help students gain quality education in the twin cities of Hyderabad and Secunderabad, the idea of an engineering and pharmacy college was conceived by Dr. G. Ganga Raju. Hence, in 1995, he established the Gokaraju Rangaraju Educational Society (GRES), in memory of his father, the Late Dr. Gokaraju Ranga Raju. It is worth noting that, apart from being a prosperous industrialist, Dr. G. Ranga Raju was also an eminent educator and evolent philanthropist.',
    'His successor, the industrious and eminent Dr. G. Ganga Raju is currently the Chairman of Laila Group of Industries and served the public by being a Member of the Parliament to the 16th Lok Sabha from 2014 to 2019. He presides over this large corporate group which has its presence in several sectors such as Pharmaceuticals, Paper, Sugar, Agribusiness, Textiles, Fishing, etc. The group is a major exporter of readymade apparel to the West, including the United States. The founding of Gokaraju Rangaraju Educational Society (GRES) in 1995 helped facilitate the establishment of the Gokaraju Rangaraju Institute of Engineering and Technology in 1997. Subsequently, to provide quality education in the field of pharmacy, the Gokaraju Rangaraju College of Pharmacy was established in 2003.',
  ],
}

export const vicePresidentData = {
  name: 'Sri G.V.K. Ranga Raju',
  title: 'Vice President',
  institution: 'Gokaraju Rangaraju College of Pharmacy',
  img: 'https://grcp.ac.in/images/Ranga_Raju.png',
  qualifications: ['Chemical Engineer'],
  bio: [
    "Continuing this epic journey, Sri G.V.K. Ranga Raju, the eldest scion of Dr. G. Ganga Raju has carved a name for himself in the field of business. A Chemical engineer by profession, he is deeply committed to the cause of education and is hence involved in the routine affairs of the Gokaraju Rangaraju Educational Society (GRES). His inherent business acumen and attention to detail has enabled him to be on several Boards of his family's businesses such as Pharmaceuticals, Paper, Aqua products, etc. He is also the current Managing Director of the group concern, Delta Paper Mills, which is in the business of manufacturing paper for writing and printing. A gifted orator and thinker, his ideas and vision have enabled the group to attain greater heights.",
    'His enormous experience and positive attitude are evident in the way he runs the Gokaraju Rangaraju College of Pharmacy and interacts with both the faculty, its students and their parents. He is a hard task master and does not compromise on ethics and integrity, especially in the field of education. Being futuristic and techno-driven, he is the key individual behind the adoption of cutting-edge technologies by the educational institutions under him. His belief is that learning should be experiential and constant and hence expects every student to be aware of and be exposed to the latest trends in technology relevant to their fields of study.',
  ],
}

export const principalData = {
  name: 'Dr. M. Ganga Raju',
  title: 'Professor and Principal',
  institution: 'Gokaraju Rangaraju College of Pharmacy',
  img: 'https://grcp.ac.in/images/vice_principal.png',
  qualifications: [
    'B.Pharm (2000) – Mangalore University',
    'M.Pharm (2003) – Rajiv Gandhi University of Health Sciences, Bangalore, Karnataka',
    'Ph.D. (2011) – Andhra University, Andhra Pradesh',
    'CMI Level 5 Certificate in Management and Leadership – Chartered Management Institute, U.K.',
  ],
  bio: [
    'Dr. Ganga Raju is an eminent educator currently serving as Principal and Professor at the institution. He brings over 21 years of teaching experience and maintains an active research profile with over 100 peer-reviewed articles published globally.',
    'His accolades include the Research Excellence Award from the Institute of Scholars; a Certificate of Merit from the Maulana Abdul Kalam Azad Excellence Award of Education (2020); Distinguished Professor Award from Sidra Bahuudeshiya Vikas Sanstha for 2021–22; "Young Professor of the Year Award-2021" from Global Teaching Excellence Awards; Indian Academic Leader Award (2021) from the International Research Association (U.K.); and "Innovative Professor of Innovation Award" (2021) from ASKM Innovations.',
  ],
}

// ── Governing Body ─────────────────────────────────────────────────────────────

export const governingBodyMembers = [
  {
    sno: 1,
    name: 'Dr. G. Ganga Raju',
    org: 'Chairman, Laila Group of Industries\nVijayawada, AP',
    designation: 'President',
    category: 'Chairman Society Nominee',
    photo: 'https://grcp.ac.in/images/ob_1.jpg',
  },
  {
    sno: 2,
    name: 'Sri. G.V.K.Ranga Raju',
    org: 'M.D., Delta Paper Mills, Hyderabad',
    designation: 'Vice President',
    category: 'Society Nominee',
    photo: 'https://grcp.ac.in/images/ob_2.jpg',
  },
  {
    sno: 3,
    name: 'Sri. G.Rama Raju',
    org: 'M.D.Laila Impex, Vijayawada',
    designation: 'Secretary',
    category: 'Society Nominee',
    photo: 'https://grcp.ac.in/images/ob_3.jpg',
  },
  {
    sno: 4,
    name: 'Smt. Vani Ranga Raju',
    org: 'M.D.,Ganges Valley School, Hyderabad',
    designation: 'Member',
    category: 'Society Nominee',
    photo: 'https://grcp.ac.in/images/ob_4.jpg',
  },
  {
    sno: 5,
    name: 'Prof. Jandhyala N Murthy',
    org: 'Director, GRIET., Hyderabad',
    designation: 'Member',
    category: 'Academician',
    photo: 'https://grcp.ac.in/images/ob_5.jpg',
  },
  {
    sno: 6,
    name: 'Dr. B. Manohar, M. Sc., Ph. D',
    org: 'Professor of Physical Chemistry, Osmania University, Hyderabad',
    designation: 'Member',
    category: 'Osmania University Nominee',
    photo: 'https://grcp.ac.in/images/ob_17.jpg',
  },
  {
    sno: 7,
    name: 'Dr. M. Narasimha Rao',
    org: 'Managing Director, Streek Group of Companies. Hyderabad',
    designation: 'Member',
    category: 'Nominee Pharmaceutical Industry',
    photo: 'https://grcp.ac.in/images/ob_9.jpg',
  },
  {
    sno: 8,
    name: 'Dr. M. Ganga Raju',
    org: 'Professor & Principal, Gokaraju Rangaraju College of Pharmacy, Hyderabad',
    designation: 'Member',
    category: 'Member secretary & Principal',
    photo: 'https://grcp.ac.in/images/ob_11.jpg',
  },
  {
    sno: 9,
    name: 'Dr. N V L Suvarchala Reddy V',
    org: 'Professor, Gokaraju Rangaraju College of Pharmacy, Hyderabad',
    designation: 'Member',
    category: 'Member, Senior Faculty',
    photo: 'https://grcp.ac.in/images/ob_13.jpg',
  },
  {
    sno: 10,
    name: 'Dr. M. Lakshmi Madhuri',
    org: 'Assistant Professor, Gokaraju Rangaraju College of Pharmacy, Hyderabad',
    designation: 'Member',
    category: 'Member, Senior Faculty',
    photo: 'https://grcp.ac.in/images/ob_14.jpg',
  },
]

// ── IDMC ──────────────────────────────────────────────────────────────────────

export const idmcIntro =
  "The Institutional Development and Monitoring Committee (IDMC) serves as the institution's principal academic body. Its responsibilities span supervision of academic activities, examination oversight, research promotion, and recommendations regarding faculty appointments and institutional standards."

export const idmcMembers = [
  { sno: 1, name: 'M. Padma Raju',                designation: 'Joint Secretary',       position: 'Member Secretary', email: 'mgrpharma@gmail.com'       },
  { sno: 2, name: 'Dr. M. Ganga Raju',            designation: 'Professor & Principal',  position: 'Chairperson',      email: 'ganga8000@grcp.ac.in'      },
  { sno: 3, name: 'Dr. A. Seetha Devi',           designation: 'Professor',             position: 'Co-ordinator',     email: 'seethadevi8090@grcp.ac.in' },
  { sno: 4, name: 'Dr. A. D. Pani Kumar',         designation: 'Associate Professor',   position: 'Member',           email: 'durga8017@grcp.ac.in'      },
  { sno: 5, name: 'Dr. Monika Nijhawan',          designation: 'Professor',             position: 'Member',           email: 'monika8009@grcp.ac.in'     },
  { sno: 6, name: 'Dr. N.V.L. Suvarchala Reddy V',designation: 'Professor',             position: 'Member',           email: 'suvarchala8018@grcp.ac.in' },
  { sno: 7, name: 'Dr. C. Nisha Shri',           designation: 'Associate Professor',   position: 'Member',           email: 'nisha8091@grcp.ac.in'      },
  { sno: 8, name: 'B. Satyanarayana Raju',        designation: 'Admin Assistant',       position: 'Member',           email: 'rajusatish1977@gmail.com'  },
]
